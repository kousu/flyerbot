...
class FlyerBot(slixmpp.ClientXMPP):
    ...
    def __init__(self, jid, password=None, storage=None):
        ...
        if omemo_storage:
            ...
            self.register_plugin(
                "xep_0454"
            )  # encrypted file sharing; technically can be used separately but why would you?
    ...
    async def message(self, msg):
        ...
        groupchat = msg["type"] == "groupchat" and (
            msg["from"].bare in self["xep_0045"].get_joined_rooms(self, pfrom=msg["to"])
        )
        ...
        if msg["type"] == "groupchat":
            ...
            mention = re.compile(rf"@?\b{re.escape(nickname.lower())}\b[:,]?\s*")
